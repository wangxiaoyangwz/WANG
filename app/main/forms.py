...
class PostForm(FlaskForm):#定义web表单类
    title = StringField(u'标题')
    #属性，文本字段，检测函数，输入是否为空
    body =PageDownField(u'你得到了什么?', validators=[Required()])
    #表单提交按钮
    submit = SubmitField(u'提交')


class EditProfileForm(FlaskForm):#资料编辑表单
    name=StringField(u'真实名字',validators=[Length(0,64)])#'Real name'要显示的文本框名字，validators验证函数，参数是列表，多个项，此处验证长度
    location=StringField(u'地址',validators=[Length(0,64)])#字段可选，可为空
...
class EditProfileAdminForm(FlaskForm):#管理员使用的资料编辑器
	...
	confirm=BooleanField(u'Confirmed')
